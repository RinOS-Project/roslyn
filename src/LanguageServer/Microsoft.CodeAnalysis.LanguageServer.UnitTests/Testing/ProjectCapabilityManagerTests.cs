...
public sealed class ProjectCapabilityManagerTests
{
    ...
    [Fact]
    public void TryHasCapabilityDistinguishesUnknownProjectFromMissingCapability()
    {
        var manager = new ProjectCapabilityManager();
        var knownProjectId = ProjectId.CreateNewId();
        var unknownProjectId = ProjectId.CreateNewId();
        manager.UpdateCapabilities(knownProjectId, ["Unrelated"]);

        Assert.True(manager.TryHasCapability(knownProjectId, "TestingPlatformServer", out var knownProjectHasCapability));
        Assert.False(knownProjectHasCapability);
        Assert.False(manager.TryHasCapability(unknownProjectId, "TestingPlatformServer", out _));
    }
}
