...
internal sealed class MtpTestRunner(ILoggerFactory loggerFactory)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var (discoveredTests, discoveryElapsed) = await DiscoverTestsAsync(projectOutputPath, progress, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<(ImmutableArray<MtpTestNodeUpdate> Tests, TimeSpan Elapsed)> DiscoverTestsAsync(
        string projectOutputPath,
        BufferedProgress<RunTestsPartialResult> progress,
        CancellationToken cancellationToken)
    {
        ...
        using var client = await LaunchClientAsync(projectOutputPath, cancellationToken).ConfigureAwait(false);
        ...
        return (tests, stopwatch.Elapsed);
    }
    ...
    private async Task<MtpServerClient> LaunchClientAsync(string projectOutputPath, CancellationToken cancellationToken)
    {
