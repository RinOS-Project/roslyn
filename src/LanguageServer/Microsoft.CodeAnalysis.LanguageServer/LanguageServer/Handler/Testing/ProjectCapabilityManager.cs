...
internal sealed class ProjectCapabilityManager : ILspService
{
    ...
    public bool TryHasCapability(ProjectId projectId, string capability, out bool hasCapability)
    {
        if (_projectCapabilities.TryGetValue(projectId, out var capabilities))
        {
            hasCapability = capabilities.Contains(capability);
            return true;
        }
