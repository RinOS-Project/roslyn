...
| `LanguageServer/` | ide | LSP server; canonical project loading lives in `Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/`, with shared load-status completion owned by `LoadedProject`; shared on-demand loading contracts and ordered document-context lookup live in `Protocol/Workspaces/`, and `Protocol/Handler/WorkspaceFolderTracker.cs` owns the synchronized workspace-folder snapshot. |
